...
import { useAppStore, useProductsStore, useCategoryStore, useCartStore } from "@/stores";
...
<template>
    <div class="container">
        <template class="d-flex align-items-center justify-content-center justify-content-lg-start">
            ...
            <ul class="nav col-12 col-lg-auto me-lg-auto mb-2 justify-content-center mb-md-0">
                <li v-for="(item, index) in navItems.items" :key="index">
                    <router-link :to="item.link ? item.link : '#'" :class="item.class ? item.class : null"
                        :target="item.target ? item.target : '_self'" class="nav-link px-2 link-secondary">
            ...
            <div class="dropdown">
                <button id="dropdown-toggle" class="btn dropdown-toggle" type="button" data-bs-toggle="dropdown"
                    aria-expanded="false">
                    Categories
                </button>
            ...
            <div class="container">
                <form @submit.prevent="runSearchMode()" class="search" role="search">
                    <input type="text" class="form-control" placeholder="Enter search term" aria-label="Search"
                        v-model="searchTerm" @input="() => runSearchMode()" />
                </form>
            </div>
            <!-- End Search Bar -->
            ...
            <div class="icon-cart" style="position: relative;">
                <span id="nav-cart-count">{{ carts.length }}</span>
                <router-link :to="{ name: 'CartView' }">
                    <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="cart"
                        viewBox="0 0 16 16">
            ...
            <router-link v-if="getIsAuthenticated === false" class="btn btn-success" to="/login">
                Connexion
            </router-link>
        </template>
        <div v-if="showUserNav === true && getIsAuthenticated === true" class="dropdown text-end">
            <a href="#" class="d-block link-body-emphasis text-decoration-none dropdown-toggle show"
                data-bs-toggle="dropdown" aria-expanded="true">
                <img src="https://github.com/mdo.png" alt="mdo" width="32" height="32" class="rounded-circle" />
            </a>
            <ul class="dropdown-menu text-small" data-popper-placement="bottom-end" style="
        position: absolute;
        ...
        ">
                <li v-for="(item, index) in userNavItems.items" :key="index">
                    <router-link v-if="checkDisplay(item, getIsAuthenticated, getIsAdmin)" :to="item.link ? item.link : '#'"
                        :class="item.class ? item.class : null" :target="item.target ? item.target : '_self'"
                        class="dropdown-item">
                        {{ item.name ? item.name : "link" }}
                    </router-link>
                </li>
